---
name: llm-wiki
description: Build and maintain a wiki/ the agent reads instead of re-ingesting raw docs. Use for "build a wiki", "stop re-reading docs every session". Not for source code.
---
...
**Why it works:** Raw docs reloaded every session = wasted tokens. A wiki page is a distilled artifact: a few hundred words read instead of the whole source, every session it is needed.
...
CLAUDE.md / AGENTS.md / GEMINI.md   (tells the agent: "reference wiki/ instead of raw-sources/")
```

## Setup

Create wiki directory at project root:
...
Add to the agent instruction file (`CLAUDE.md`, `AGENTS.md` for Codex, `GEMINI.md` for Gemini CLI):
...
Numbers depend on the source. Measure, don't assume: run `estimate-tokens`
before ingesting (raw doc word count × 1.3) and after (the wiki pages a
typical question actually reads × 1.3). Report both to the user.
...
- Run `/context-forge:estimate-tokens` after wiki setup to measure baseline
- Add the `wiki/` reference to the instruction file via `optimize-claudemd`
